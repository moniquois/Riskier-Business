...
//        }
        ...
        //for(int i=0; i < thePlayers.size() ;i++){
    PlayerType* aggr = new AggressivePlayer;
    PlayerType* bene = new BenevolantPlayer;
    PlayerType* hum = new HumanPlayer;
//    aggr->reinforce(thePlayers[0]);
//    aggr->attack(thePlayers[0]);
//    aggr->fortify(thePlayers[0]);
//    bene->reinforce(thePlayers[1]);
//    bene->attack(thePlayers[1]);
//    bene->fortify(thePlayers[1]);
    thePlayers[0]->setStrategy(aggr);
    thePlayers[1]->setStrategy(bene);
  // thePlayers[2]->setStrategy(hum);
    thePlayers[0]->playerTurn();
   // thePlayers[1]->playerTurn();
    thePlayers[0]->playerTurn();
   // thePlayers[2]->playerTurn();
   // PlayerType* p =  thePlayers[0]->getStrategy();
   
   // p->attack(thePlayers[0]);
   // thePlayers[0]->reinforce();
    //thePlayers[0]->attack();
   // thePlayers[0]->fortify();
